...
if not os.getenv("API_URL"):
    st.warning("API_URL not set; defaulting to http://127.0.0.1:8000")
        ...
        if st.button("Login"):
            ...
            if res.status_code==200:
                ...
                try:
                    ...
                    detail = f"Login failed (status {res.status_code}): {res.text}"
                st.error(detail)
        ...
        if st.button("Signup"):
            ...
            if res.status_code==200:
                ...
                try:
                    ...
                    detail = f"Signup failed (status {res.status_code}): {res.text}"
                st.error(detail)
        ...
        if uploaded_file:
            ...
            if res.status_code==200:
                ...
                try:
                    ...
                    detail = f"Upload failed (status {res.status_code}): {res.text}"
                st.error(detail)
        else:
...
def chat_interface():
    ...
    if st.button("Send"):
        ...
        if res.status_code==200:
            ...
            try:
                detail = res.json().get("detail", "Something is wrong.")
            except ValueError:
                detail = f"Something is wrong (status {res.status_code}): {res.text}"
            st.error(detail)
